...
public class dna_JDBC_String {

//    public static void main(String[] args) {
//        dna_JDBC_String str = new dna_JDBC_String();
//        str.JDBC_patient();
...
//            state = conn.createStatement(); //sql 문을 실행하기 위해 conn 연결 정보를 state로 생성
            ...
            String sql_sec;
            sql_sec = "SELECT p.id FROM patient p WHERE p.hospital_no = \""+hospital_no+"\" and p.phone = \""+phone+"\";";

            stmt = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
            rsCount = stmt.executeQuery(sql_sec);
            rsCount.last();
            int rowCount = rsCount.getRow();

            System.out.println("RowCount : "+rowCount);

            patient = new String[rowCount];
